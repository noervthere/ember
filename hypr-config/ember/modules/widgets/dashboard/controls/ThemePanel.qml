...
Item {
    ...
    FileView {
        ...
        JsonAdapter {
            // Keep in sync with Wallpaper.qml adapter so writeAdapter() does not drop keys
    ...
    // Prefer the live wallpaper manager when available so changes apply immediately
    function setParallaxEnabled(enabled) {
        if (GlobalStates.wallpaperManager) {
            GlobalStates.wallpaperManager.parallaxEnabled = enabled;
        } else {
            wallpaperConfig.adapter.parallaxEnabled = enabled;
            wallpaperConfig.writeAdapter();
        }
    }

    function setParallaxWorkspaceZoom(zoom) {
        const clamped = Math.max(0.1, Math.min(2.0, zoom));
        if (GlobalStates.wallpaperManager) {
            GlobalStates.wallpaperManager.parallaxWorkspaceZoom = clamped;
        } else {
            wallpaperConfig.adapter.parallaxWorkspaceZoom = clamped;
            wallpaperConfig.writeAdapter();
        }
    }
                ...
                ColumnLayout {
                    ...
                    Item {
                        ...
                        ColumnLayout {
                            ...
                            // Workspace wallpaper parallax (horizontal)
                            RowLayout {
                                Layout.fillWidth: true
                                spacing: 8
                                ...
                                Switch {
                                    ...
                                    checked: {
                                        if (GlobalStates.wallpaperManager)
                                            return GlobalStates.wallpaperManager.parallaxEnabled;
                                        return wallpaperConfig.adapter.parallaxEnabled;
                                    }
                                    ...
                                    onCheckedChanged: {
                                        const current = GlobalStates.wallpaperManager ? GlobalStates.wallpaperManager.parallaxEnabled : wallpaperConfig.adapter.parallaxEnabled;
                                        if (checked !== current)
                                            root.setParallaxEnabled(checked);
                                    }

                                    indicator: Rectangle {
                                        implicitWidth: 40
                                        implicitHeight: 20
                                        x: parallaxSwitch.leftPadding
                                        y: parent.height / 2 - height / 2
                                        radius: height / 2
                                        color: parallaxSwitch.checked ? Styling.srItem("overprimary") : Colors.surfaceBright
                                        border.color: parallaxSwitch.checked ? Styling.srItem("overprimary") : Colors.outline
                                        ...
                                        Rectangle {
                                            x: parallaxSwitch.checked ? parent.width - width - 2 : 2
                                            y: 2
                                            width: parent.height - 4
                                            height: width
                                            radius: width / 2
                                            color: parallaxSwitch.checked ? Colors.background : Colors.overSurfaceVariant
                            ...
                            // Wallpaper zoom controls parallax travel (end-4 style workspaceZoom)
                            RowLayout {
                                Layout.fillWidth: true
                                spacing: 8
                                opacity: (GlobalStates.wallpaperManager ? GlobalStates.wallpaperManager.parallaxEnabled : wallpaperConfig.adapter.parallaxEnabled) ? 1.0 : 0.45
                                enabled: GlobalStates.wallpaperManager ? GlobalStates.wallpaperManager.parallaxEnabled : wallpaperConfig.adapter.parallaxEnabled
                                ...
                                StyledSlider {
                                    id: parallaxZoomSlider
                                    Layout.fillWidth: true
                                    Layout.preferredHeight: 20
                                    progressColor: Styling.srItem("overprimary")
                                    // Map slider 0..1 → zoom 0.10..2.00 (10%–200%), matching end-4
                                    readonly property real minZoom: 0.10
                                    readonly property real maxZoom: 2.00
                                    tooltipText: `${Math.round((minZoom + value * (maxZoom - minZoom)) * 100)}%`
                                    scroll: true
                                    stepSize: 1 / 190  // 1% steps across 10–200
                                    snapMode: "always"

                                    readonly property real configValue: {
                                        const zoom = GlobalStates.wallpaperManager ? GlobalStates.wallpaperManager.parallaxWorkspaceZoom : (wallpaperConfig.adapter.parallaxWorkspaceZoom || 1.07);
                                        return Math.max(0, Math.min(1, (zoom - minZoom) / (maxZoom - minZoom)));
                                    }

                                    onConfigValueChanged: {
                                        if (Math.abs(value - configValue) > 0.0005)
                                            value = configValue;
                                    }
                                    ...
                                    onValueChanged: {
                                        const zoom = Math.round((minZoom + value * (maxZoom - minZoom)) * 100) / 100;
                                        const current = GlobalStates.wallpaperManager ? GlobalStates.wallpaperManager.parallaxWorkspaceZoom : wallpaperConfig.adapter.parallaxWorkspaceZoom;
                                        if (Math.abs(zoom - current) > 0.001)
                                            root.setParallaxWorkspaceZoom(zoom);
                                    }
                                }
