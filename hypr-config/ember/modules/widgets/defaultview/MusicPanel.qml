...
Item {
    ...
    StyledRect {
        ...
        Image {
            ...
            opacity: 0.85 // dimmed slightly so the rim doesn't outshine the frosted panes
            asynchronous: true
        ...
        MultiEffect {
            ...
            blurMax: 28
            blur: 0.85
            brightness: -0.12 // dim the real backdrop a touch
            saturation: 0.12  // blur tends to wash colour out - claw a little back
    ...
    RowLayout {
        ...
        StyledRect {
            ...
            // Real frosted-glass backdrop: window into the shared blur
            FrostedWindow {
                anchors.fill: parent
                z: -1
            ...
            Item {
                ...
                RotationAnimation {
                    ...
                    // Whenever continuous spin stops (paused, track ended, or panel
                    // hidden) ease the disc back to its upright resting position
                    // instead of freezing wherever it happened to stop.
                    onRunningChanged: {
                        if (running) {
                            discSpinBackAnim.stop();
                            return;
                        }
                        const current = ((discContainer.rotation % 360) + 360) % 360;
                        discContainer.rotation = current;
                        if (current < 0.5) return;
                        if (panel.panelVisible) {
                            discSpinBackAnim.from = current;
                            discSpinBackAnim.to = current > 180 ? 360 : 0;
                            discSpinBackAnim.start();
                        } else {
                            discContainer.rotation = 0; // hidden: snap, no need to animate
                        }
                    }
                }
                ...
                NumberAnimation {
                    ...
                    onStopped: {
                        if (discContainer.rotation === 360)
                            discContainer.rotation = 0;
                    }
                }
            }

            // --- Musical particles - drift in an orbit around the disc while playing ---
            Item {
                id: noteParticles
                anchors.centerIn: parent
                width: 1; height: 1
                z: 6
                visible: opacity > 0
                opacity: (panel.panelVisible && panel.isPlaying) ? 1.0 : 0.0
                ...
                Repeater {
                    ...
                    delegate: Item {
                        ...
                        // Each particle gets its own radius, speed and starting
                        // angle so they don't all move in lockstep.
                        readonly property real orbitRadius: panel.discSize / 2 + 6 + (index % 4) * 5 + Math.random() * 4
                        readonly property int spinDuration: 9000 + Math.random() * 6000
                        readonly property real startAngle: (index / 7) * 360 + Math.random() * 20
                        readonly property bool isNote: index % 2 === 0

                        Component.onCompleted: rotation = startAngle

                        RotationAnimation {
                            target: noteOrbit; property: "rotation"
                            from: noteOrbit.rotation; to: noteOrbit.rotation + 360
                            duration: noteOrbit.spinDuration
                            loops: Animation.Infinite
                            running: panel.panelVisible && panel.isPlaying
                        }
                        ...
                        Text {
                            id: noteGlyph
                            x: noteOrbit.orbitRadius
                            y: -height / 2
                            text: noteOrbit.isNote ? "♪" : "♫"
                            font.pixelSize: 10 + (index % 3) * 2
                            color: noteOrbit.isNote ? Colors.primary : Colors.secondary
                            opacity: 0.5
                            rotation: -noteOrbit.rotation // keep glyph upright while orbiting

                            SequentialAnimation on opacity {
                                running: panel.panelVisible && panel.isPlaying
                                loops: Animation.Infinite
                                NumberAnimation { to: 0.85; duration: 1200 + Math.random() * 800; easing.type: Easing.InOutSine }
                                NumberAnimation { to: 0.3; duration: 1200 + Math.random() * 800; easing.type: Easing.InOutSine }
                            }
                        }
                    }
                }
            ...
            Item {
                ...
                // Counterweight - small drum behind the pivot, balances the arm
                Rectangle {
                    x: 32; y: 0
                    width: 9; height: 12
                    radius: 4
                ...
                // Arm rod + headshell, laid out straight down in local space so the
                // headshell always sits exactly at the rod's tip - the whole group
                // is tilted to its resting angle here, then swung further onto the
                // record by the parent Rotation transform when playing.
                Item {
                    id: armAssembly
                    x: 30; y: 8
                    width: 1; height: 1
                    ...
                    // thin highlight for a touch of dimension
                    Rectangle {
                        x: -1.25; y: 2
                        width: 1.5; height: 44
                        radius: 1
                    ...
                    // Headshell (cartridge housing)
                    Rectangle {
                        id: headshell
                        x: -8; y: 44
                        width: 16; height: 9
                        radius: 3
                        color: Qt.lighter(Colors.secondary, 1.4)
                        rotation: -8
                    }

                    // Stylus - small accent-colored tip
                    Rectangle {
                        x: -1; y: 52
                        width: 2; height: 6
                        radius: 1
                ...
                // Pivot base + center screw, drawn last so it sits on top of the rod
                Rectangle {
                    x: 30 - 7; y: 8 - 7
                    width: 14; height: 14
                    radius: 7
                ...
                Rectangle {
                    ...
                    width: 4; height: 4
                    radius: 2
                    color: Qt.darker(Colors.secondary, 1.6)
                    opacity: 0.8
                }
            }
        }
        ...
        ColumnLayout {
            ...
            // TOP ROW: Now Playing Pill (title + pet mascot share one pane)
            RowLayout {
                Layout.fillWidth: true
                Layout.preferredHeight: 74
                spacing: 6

                // PILL 2: Now Playing Pill (merged track details + pet mascot)
                StyledRect {
                    id: nowPlayingPill
                    variant: "pane"
                    backgroundOpacity: 0.72
                    ...
                    RowLayout {
                        ...
                        // Thin divider - keeps the two sections legible now
                        // that they share one background instead of a seam.
                        Rectangle {
        ...
        StyledRect {
            ...
            // Parallax transform on lyrics section (matches middle section)
            transform: Translate {
                x: panel._mouseNormX * 3
                y: panel._mouseNormY * 2
            }
