...
import "defaults/onlineWallpapers.js" as OnlineWallpapersDefaults
import "defaults/lyrics.js" as LyricsDefaults
...
Singleton {
    ...
    property bool onlineWallpapersReady: false
    property bool lyricsReady: false
    property bool keybindsInitialLoadComplete: false

    property bool initialLoadComplete: themeReady && barReady && workspacesReady && overviewReady && notchReady && compositorReady && performanceReady && weatherReady && desktopReady && lockscreenReady && prefixReady && systemReady && dockReady && aiReady && generalReady && onlineWallpapersReady && lyricsReady
    ...
    FileView {
        ...
        adapter: JsonAdapter {
            ...
            // Ember Search
            property int searchDebounceMs: 300
            property int searchMaxResults: 20
            property string defaultInstallMethod: "flake"
            property string nixConfigPath: "/etc/nixos/configuration.nix"
            property string configEditor: "nano"
            property bool idleHoverOpensSearch: true
        }
    }
    ...
    // ============================================
    // ONLINE WALLPAPERS MODULE
    // ============================================
    FileView {
        id: onlineWallpapersLoader
        path: root.configDir + "/online_wallpapers.json"
        atomicWrites: true
        watchChanges: true
        onLoaded: {
            if (!root.onlineWallpapersReady) {
                validateModule("onlineWallpapers", onlineWallpapersLoader, OnlineWallpapersDefaults.data, () => {
                    root.onlineWallpapersReady = true;
                });
            }
        }
        onLoadFailed: {
            if (error.toString().includes("FileNotFound") && !root.onlineWallpapersReady) {
                handleMissingConfig("onlineWallpapers", onlineWallpapersLoader, OnlineWallpapersDefaults.data, () => {
                    root.onlineWallpapersReady = true;
                });
            }
        }
        ...
        onPathChanged: reload()
        onAdapterUpdated: {
            if (root.onlineWallpapersReady && !root.pauseAutoSave) {
                onlineWallpapersLoader.writeAdapter();
            }
        }

        adapter: JsonAdapter {
            property string defaultSource: "wallhaven"
            property int resultsPerPage: 24
            property string cacheDir: "~/.cache/ambxst/online_wallpapers"
            property string wallhavenApiKey: ""
        }
    }

    // Lyrics configuration (lyrics.json)
    FileView {
        id: lyricsLoader
        path: root.configDir + "/lyrics.json"
        function reload() {
            root.pauseAutoSave = true;
            read();
            root.pauseAutoSave = false;
        ...
        onLoaded: {
            if (!root.lyricsReady) {
                validateModule("lyrics", lyricsLoader, LyricsDefaults.data, () => {
                    root.lyricsReady = true;
                });
            }
        }
        onLoadFailed: error => {
            if (error.toString().includes("FileNotFound") && !root.lyricsReady) {
                handleMissingConfig("lyrics", lyricsLoader, LyricsDefaults.data, () => {
                    root.lyricsReady = true;
                });
            }
        }
        ...
        onPathChanged: reload()
        onAdapterUpdated: {
            if (root.lyricsReady && !root.pauseAutoSave) {
                lyricsLoader.writeAdapter();
            }
        }
    ...
    // Online Wallpapers configuration
    property QtObject onlineWallpapers: onlineWallpapersLoader.adapter

    // Lyrics configuration
    property QtObject lyrics: lyricsLoader.adapter
