...
PanelWindow {
    ...
    // Workspace parallax (horizontal only) — pans wallpaper with active workspace
    property alias parallaxEnabled: wallpaperAdapter.parallaxEnabled
    property alias parallaxWorkspaceZoom: wallpaperAdapter.parallaxWorkspaceZoom
    // How strongly desktop widgets stick to the wallpaper pan (end-4 widgetsFactor)
    property alias parallaxWidgetsFactor: wallpaperAdapter.parallaxWidgetsFactor
    property int thumbnailsVersion: 0

    // Live image pan offset relative to the centered "rest" pose.
    // Desktop clock multiplies this by parallaxWidgetsFactor / zoom to ride the wallpaper.
    property real parallaxOffsetX: 0
    property real parallaxOffsetY: 0

    function publishParallaxOffset(ox, oy) {
        parallaxOffsetX = ox;
        parallaxOffsetY = oy;
        // Publish full state from THIS screen's wallpaper so the clock does not
        // wait on wallpaperManager init (that delay caused follow-then-snap).
        GlobalStates.setWallpaperParallaxOffset(currentScreenName, ox, oy, parallaxEnabled, parallaxWorkspaceZoom, parallaxWidgetsFactor);
    }

    onParallaxEnabledChanged: publishParallaxOffset(parallaxEnabled ? parallaxOffsetX : 0, parallaxEnabled ? parallaxOffsetY : 0)
    onParallaxWorkspaceZoomChanged: publishParallaxOffset(parallaxOffsetX, parallaxOffsetY)
    onParallaxWidgetsFactorChanged: publishParallaxOffset(parallaxOffsetX, parallaxOffsetY)
    onCurrentScreenNameChanged: publishParallaxOffset(parallaxOffsetX, parallaxOffsetY)

    // Per-monitor workspace range for parallax (mirrors end-4 Background.qml)
    readonly property var compositorMonitor: {
        // Depend on monitors list so bindings refresh on workspace switches
        const _monitors = AxctlService.monitors.values;
        return AxctlService.monitorFor(wallpaper.screen);
    }
    readonly property var relevantWindows: {
        const _clients = AxctlService.clients.values;
        const mon = wallpaper.compositorMonitor;
        if (!mon)
            return [];
        return AxctlService.clients.values.filter(function (win) {
            return win.monitor == mon.id && win.workspace && win.workspace.id >= 0;
        }).sort(function (a, b) {
            return a.workspace.id - b.workspace.id;
        });
    }
    readonly property int firstWorkspaceId: relevantWindows.length > 0 ? relevantWindows[0].workspace.id : 1
    readonly property int lastWorkspaceId: relevantWindows.length > 0 ? relevantWindows[relevantWindows.length - 1].workspace.id : Math.max(1, Config.workspaces.shown)
    readonly property int workspaceChunkSize: Config.workspaces.shown > 0 ? Config.workspaces.shown : 10
    readonly property int totalWorkspaces: Math.max(workspaceChunkSize, Math.ceil(lastWorkspaceId / workspaceChunkSize) * workspaceChunkSize)
    readonly property int activeWorkspaceId: {
        const _monitors = AxctlService.monitors.values;
        const mon = wallpaper.compositorMonitor;
        return (mon && mon.activeWorkspace && mon.activeWorkspace.id) ? mon.activeWorkspace.id : 1;
    }
    ...
    function setExternalWallpaper(path, targetScreen = null) {
        if (GlobalStates.wallpaperManager && GlobalStates.wallpaperManager !== wallpaper) {
            GlobalStates.wallpaperManager.setExternalWallpaper(path, targetScreen);
            return;
        }

        console.log("setExternalWallpaper called with:", path, "for screen:", targetScreen);
        initialLoadCompleted = true;
        if (targetScreen) {
            let perScreen = Object.assign({}, wallpaperConfig.adapter.perScreenWallpapers || {});
            perScreen[targetScreen] = path;
    ...
    FileView {
        ...
        JsonAdapter {
            ...
            property bool parallaxEnabled: false
            property real parallaxWorkspaceZoom: 1.07
            property real parallaxWidgetsFactor: 1.2
    ...
    component WallpaperImage: Item {
        ...
        Component {
            ...
            Item {
                ...
                property bool parallaxActive: wallpaper.parallaxEnabled

                // Natural image size (updated when the probe/image loads)
                property int wallpaperWidth: wallpaper.width
                property int wallpaperHeight: wallpaper.height

                // Cover scale so the image fills the screen, then apply workspace zoom for pan room
                // (same math as end-4 Background.qml parallax)
                readonly property real parallaxZoom: Math.max(1.0, wallpaper.parallaxWorkspaceZoom || 1.07)
                readonly property real minSuitableScale: {
                    if (wallpaperWidth <= 0 || wallpaperHeight <= 0)
                        return 1.0;
                    return Math.max(width / wallpaperWidth, height / wallpaperHeight);
                }
                readonly property real effectiveWallpaperScale: parallaxActive ? (minSuitableScale * parallaxZoom) : 1.0
                readonly property real scaledWallpaperWidth: parallaxActive ? (wallpaperWidth * effectiveWallpaperScale) : width
                readonly property real scaledWallpaperHeight: parallaxActive ? (wallpaperHeight * effectiveWallpaperScale) : height
                readonly property real parallaxTotalPixelsX: Math.max(0, scaledWallpaperWidth - width)

                // 0 = left edge of wallpaper, 1 = right edge — driven by active workspace index
                readonly property real workspaceFraction: {
                    if (!parallaxActive)
                        return 0.5;
                    const total = wallpaper.totalWorkspaces;
                    if (total <= 1)
                        return 0.5;
                    const workspaceIndex = Math.max(0, wallpaper.activeWorkspaceId - 1);
                    return Math.max(0, Math.min(1, workspaceIndex / (total - 1)));
                }
                ...
                // Hidden probe to read natural image dimensions without forcing a decode size
                Image {
                    id: sizeProbe
                    visible: false
                    asynchronous: true
                    ...
                    onStatusChanged: {
                        if (status === Image.Ready && sourceSize.width > 0 && sourceSize.height > 0) {
                            staticImageRoot.wallpaperWidth = sourceSize.width;
                            staticImageRoot.wallpaperHeight = sourceSize.height;
                        }
                    }
                    onSourceChanged: {
                        // Reset to screen size until the new image reports its dimensions
                        staticImageRoot.wallpaperWidth = wallpaper.width;
                        staticImageRoot.wallpaperHeight = wallpaper.height;
                    }
                }
                ...
                Image {
                    ...
                    // Non-parallax: fill the screen exactly (previous behavior)
                    // Parallax: oversized image that pans horizontally with the workspace
                    width: staticImageRoot.parallaxActive ? staticImageRoot.scaledWallpaperWidth : staticImageRoot.width
                    height: staticImageRoot.parallaxActive ? staticImageRoot.scaledWallpaperHeight : staticImageRoot.height
                    x: {
                        if (!staticImageRoot.parallaxActive)
                            return 0;
                        if (staticImageRoot.width > width)
                            return (staticImageRoot.width - width) / 2;
                        return -staticImageRoot.parallaxTotalPixelsX * staticImageRoot.workspaceFraction;
                    }
                    y: {
                        if (!staticImageRoot.parallaxActive)
                            return 0;
                        // Horizontal-only: keep the image vertically centered
                        return (staticImageRoot.height - height) / 2;
                    }

                    // Publish offset relative to the centered rest pose (end-4 wallpaperTotalOffset*)
                    readonly property real baseOffsetX: (staticImageRoot.width - width) / 2
                    readonly property real baseOffsetY: (staticImageRoot.height - height) / 2
                    readonly property real totalOffsetX: staticImageRoot.parallaxActive ? (x - baseOffsetX) : 0
                    readonly property real totalOffsetY: staticImageRoot.parallaxActive ? (y - baseOffsetY) : 0

                    onTotalOffsetXChanged: wallpaper.publishParallaxOffset(totalOffsetX, totalOffsetY)
                    onTotalOffsetYChanged: wallpaper.publishParallaxOffset(totalOffsetX, totalOffsetY)
                    Component.onCompleted: wallpaper.publishParallaxOffset(totalOffsetX, totalOffsetY)
                    Component.onDestruction: {
                        if (!staticImageRoot.parallaxActive)
                            wallpaper.publishParallaxOffset(0, 0);
                    }

                    sourceSize.width: staticImageRoot.parallaxActive ? Math.ceil(staticImageRoot.scaledWallpaperWidth) : wallpaper.width
                    sourceSize.height: staticImageRoot.parallaxActive ? Math.ceil(staticImageRoot.scaledWallpaperHeight) : wallpaper.height

                    Behavior on x {
                        enabled: staticImageRoot.parallaxActive && Config.animDuration > 0
                        NumberAnimation {
                            duration: Math.max(Config.animDuration, 600)
                            easing.type: Easing.OutCubic
                        }
                    }
