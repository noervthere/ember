...
Item {
    ...
    onIsMouseOverNotchChanged: {
        if (screenNotchOpen || menuCooldown) {
            root.hoverActive = false;
            return;
        }
        if (isMouseOverNotch) {
    ...
    readonly property bool isSearchHoverExpanded: notchContainer.isSearchHoverExpanded
}
