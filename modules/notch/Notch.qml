...
Item {
    ...
    readonly property bool isHoverExpanded: stackViewInternal.currentItem && (stackViewInternal.currentItem.mediaHoverExpanded === true || stackViewInternal.currentItem._collapseKeepAlive === true)
    readonly property bool isSearchHoverExpanded: stackViewInternal.currentItem && stackViewInternal.currentItem.isSearchHoverExpanded === true

    property int defaultHeight: Config.showBackground ? (screenNotchOpen || hasActiveNotifications || isHoverExpanded ? Math.max(stackContainer.height, 44) : 44) : (screenNotchOpen || hasActiveNotifications || isHoverExpanded ? Math.max(stackContainer.height, 40) : 40)
    property int islandHeight: screenNotchOpen || hasActiveNotifications || isHoverExpanded ? Math.max(stackContainer.height, 36) : 36
    ...
    // Disabled debug messages (keeps console clean)
    onWidthChanged: () => {}
    onHeightChanged: () => {}
    ...
    Behavior on implicitWidth {
        ...
        NumberAnimation {
            ...
            easing.type: isExpanded || isHoverExpanded ? Easing.OutBack : Easing.OutQuart
            easing.overshoot: isExpanded || isHoverExpanded ? 1.2 : 1.0
        }
    }
    ...
    StyledRect {
        ...
        property int defaultRadius: Config.roundness > 0 ? (screenNotchOpen || hasActiveNotifications || isHoverExpanded ? Config.roundness + 20 : Config.roundness + 4) : 0
        ...
        Behavior on bottomLeftRadius {
            ...
            NumberAnimation {
                duration: Config.animDuration
                easing.type: screenNotchOpen || hasActiveNotifications || isHoverExpanded ? Easing.OutBack : Easing.OutQuart
                easing.overshoot: screenNotchOpen || hasActiveNotifications || isHoverExpanded ? 1.2 : 1.0
            }
        }
    ...
    Item {
        ...
        // Include isHoverExpanded so expanded media/search keeps large rounded corners
        // (without this, sliding panels show squared edges against the frame).
        property int defaultRadius: Config.roundness > 0 ? (screenNotchOpen || hasActiveNotifications || isHoverExpanded ? Config.roundness + 20 : Config.roundness + 4) : 0
        property int islandRadius: Config.roundness > 0 ? (screenNotchOpen || hasActiveNotifications || isHoverExpanded ? Config.roundness + 20 : Config.roundness + 4) : 0
        ...
        Item {
            ...
            // Always round-clip content to the notch radii so train slides / expands
            // never flash squared corners outside the island frame.
            readonly property bool needsRoundClip: Config.roundness > 0 && (isHoverExpanded || screenNotchOpen || hasActiveNotifications)
            layer.enabled: needsRoundClip || transitionBlur > 0.0
            layer.smooth: true
            layer.effect: MultiEffect {
                blurEnabled: Config.performance.blurTransition && stackContainer.transitionBlur > 0.0
                blurMax: 64
                blur: Math.min(Math.max(stackContainer.transitionBlur, 0.0), 1.0)
                maskEnabled: stackContainer.needsRoundClip
                maskSource: stackRoundMask
                maskThresholdMin: 0.5
                maskThresholdMax: 1.0
                maskSpreadAtMin: 1.0
            }

            // Invisible round-rect mask matching notchRect corner radii
            Item {
                id: stackRoundMask
                anchors.fill: parent
                visible: false
