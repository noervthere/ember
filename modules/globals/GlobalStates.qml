...
Singleton {
    ...
    // Per-screen wallpaper parallax state for desktop widgets (end-4 style).
    // screen name → { x, y, enabled, zoom, widgetsFactor }
    property var wallpaperParallaxOffsets: ({})
    property int wallpaperParallaxRev: 0

    function setWallpaperParallaxOffset(screenName, ox, oy, enabled, zoom, widgetsFactor) {
        if (!screenName)
            return;
        let next = Object.assign({}, root.wallpaperParallaxOffsets);
        let prev = next[screenName];
        let en = !!enabled;
        let z = (zoom !== undefined && zoom !== null) ? zoom : 1.07;
        let wf = (widgetsFactor !== undefined && widgetsFactor !== null) ? widgetsFactor : 1.2;
        if (prev && prev.enabled === en && Math.abs(prev.x - ox) < 0.01 && Math.abs(prev.y - oy) < 0.01 && Math.abs(prev.zoom - z) < 0.0001 && Math.abs(prev.widgetsFactor - wf) < 0.0001)
            return;
        next[screenName] = {
            "x": ox,
            "y": oy,
        ...
        root.wallpaperParallaxOffsets = next;
        root.wallpaperParallaxRev++;
    }

    function wallpaperParallaxState(screenName) {
        let _ = root.wallpaperParallaxRev;
        return root.wallpaperParallaxOffsets[screenName] || null;
    }

    function wallpaperParallaxOffsetX(screenName) {
        let o = root.wallpaperParallaxState(screenName);
        return o ? o.x : 0;
    }

    function wallpaperParallaxOffsetY(screenName) {
        let o = root.wallpaperParallaxState(screenName);
        return o ? o.y : 0;
    }
    ...
    property string onlineWallpaperDownloadingUrl: ""
    property string onlineWallpaperSource: "wallhaven"

    function clearWallpaperState() {
        wallpaperSelectedIndex = -1;
        onlineWallpaperDownloadingUrl = "";
    }
